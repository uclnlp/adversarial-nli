...
Code for generating Adversarial examples for Neural NLI Models, by violating logic constraints.
...
entails or contradicts the other, and the NLI model says "*sentence one* contradicts *sentence two*, but *sentence two* does not contradicts *sentence one*".

You know the model is making a mistake, because *contradiction is symmetric*, and if *sentence one* contradicts *sentence two*, then *sentence two* should
contradict *sentence one* as well - if the model does not agree on this, then it is clearly making a mistake somewhere.
...
- A method for systematically generating *adversarial examples*, by violating such constraints,
- A set of *adversarial datasets* for measuring the robustness of NLI models to such adversarial examples, and
